...
def showUniversities():
	...
	if 'username' not in login_session:
		return render_template('public_universities.html',universities=universities)
	else:
...
def newUniversity():
	...
	if request.method == 'POST':
		newUniversity = University(name = request.form['name'],user_id=login_session['user_id'])
...
def showCurriculum(university_id):
	...
	if 'username' not in login_session:
		return render_template('public_courses.html',categories=categories, items=items, quantity=quantity)
	else:
		return render_template('curriculum.html', courses = courses, university = university, creator=creator)
...
def newCourse(university_id):
	...
	university = session.query(University).filter_by(id = university_id).one()
	if request.method == 'POST':
		newCourse = Course(name = request.form['name'], description = request.form['description'], professor = request.form['professor'], school = request.form['school'], university_id = university_id, user_id=university.user_id)
